...
const redirectMainPage = () => window.location.href = '/';
...
<template>
  <div class="header w-screen m-auto h-16 bg-[#032541] flex">
    <div class="header-left flex gap-5 items-center justify-center w-6/12 h-full">
      <img class="img h-5 cursor-pointer" :src="icon" @click="redirectMainPage" />
      <ul class="menu-item-wrapper flex gap-5" v-for="(item, index) in menuItems" :key="index">
        <li>
          <a class="menu-item text-base font-bold cursor-pointer relative" :href="item.url">
            {{ item.name }}
          </a>
        </li>
      </ul>
